	...
	print(plate)
		...
		for _,v in pairs(vehicules) do
			if(plate == v.plate)then
				...
				MySQL.Sync.execute("UPDATE owned_vehicles SET vehicle =@vehprop WHERE plate=@id",{['@vehprop'] = vehprop, ['@id'] = plate})
				isFound = true
				break
			end
		end
...
AddEventHandler('eden_garage:modifystate', function(vehicleProps, state)
	...
	for _,v in pairs(vehicules) do
		if(plate == v.plate)then
			local plate = vehicleProps.plate
			MySQL.Sync.execute("UPDATE owned_vehicles SET state =@state WHERE plate=@id",{['@state'] = state , ['@id'] = plate})
			break
		end		
	end
end)	
